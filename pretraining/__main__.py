...
from pretraining.experiment import Experiment
...
if debug:
    print("debug mode")
    if param["wandbosity"]:
        wandb.init(project=regime, entity=wentity, config=param, name=str(jobid))
    ...
    def run_sweep():
        ...
        experiment.run()
    sweep_id = wandb.sweep(hyperparameters(param), project=regime+"_sweep", entity=wentity)
